...
export function PortfolioCard({ item }: PortfolioCardProps): JSX.Element {
  ...
  const resolvedSrc = buildRenderImageUrl(rawImageSrc, { width: 960, quality: 70 }) ?? rawImageSrc;
  const imageSrc = resolvedSrc || IMAGE_FALLBACK_PIXEL;
  const imageSrcSet = buildRenderSrcSet(rawImageSrc, CARD_IMAGE_WIDTHS, { quality: 70 });
  const card = (
